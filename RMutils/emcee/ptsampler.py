...
class PTSampler(Sampler):
    ...
    def sample(
        self, p0, lnprob0=None, lnlike0=None, iterations=1, thin=1, storechain=True
    ):
        """
        ...
        * ``lnlike`` the current likelihood values for the walkers.
        ...
        if lnprob0 is None or lnlike0 is None:
            ...
            logls = np.array([r[0] for r in results]).reshape(
                (self.ntemps, self.nwalkers)
            )
            logps = np.array([r[1] for r in results]).reshape(
                (self.ntemps, self.nwalkers)
            )
        ...
        if storechain:
            ...
            if self._chain is None:
                ...
                self._chain = np.concatenate(
                    (
                        self._chain,
                        np.zeros((self.ntemps, self.nwalkers, nsave, self.dim)),
                    ),
                    axis=2,
                )
                self._lnprob = np.concatenate(
                    (self._lnprob, np.zeros((self.ntemps, self.nwalkers, nsave))),
                    axis=2,
                )
                self._lnlikelihood = np.concatenate(
                    (self._lnlikelihood, np.zeros((self.ntemps, self.nwalkers, nsave))),
                    axis=2,
                )
        ...
        for i in range(iterations):
            for j in [0, 1]:
                ...
                if self.pool is None:
                    ...
                    results = list(self.pool.map(fn, qs.reshape((-1, self.dim))))
                ...
                qslnprob = qslogls * self.betas.reshape((self.ntemps, 1)) + qslogps

                logpaccept = self.dim * np.log(zs) + qslnprob - lnprob[:, jupdate::2]
                logrs = np.log(
                    np.random.uniform(
                ...
                lnprob[:, jupdate::2].reshape((-1,))[accepts] = qslnprob.reshape((-1,))[
                    accepts
                ]
                logl[:, jupdate::2].reshape((-1,))[accepts] = qslogls.reshape((-1,))[
                    accepts
                ]
    ...
    def _temperature_swaps(self, p, lnprob, logl):
        ...
        for i in range(ntemps - 1, 0, -1):
            ...
            lnprob[i, iperm[asel]] = (
                lnprob[i - 1, i1perm[asel]] - dbeta * logl[i - 1, i1perm[asel]]
            )
