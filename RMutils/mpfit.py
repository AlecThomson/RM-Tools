...
Perform Levenberg-Marquardt least-squares minimization, based on MINPACK-1.
 ...
 Sergey Koposov converted the Mark's Python version from Numeric to numpy
  ...
  UPDATE 2016-05-15: Cormac Purcell updated this version to use the latest
 ...
 in Scientific.Functions.Least_squares.
...
class mpfit:
    ...
    (blas_enorm64,) = blas.get_blas_funcs(
            ...
            while 1:
                ...
                if (qanylim == 0) and (qminmax == 0):
                    ...
                    if qanylim:
                        ...
                        whl = (
                            numpy.nonzero(((dwa1 != 0.0) & qllim) & ((x + wa1) < llim))
                        )[0]
                        if len(whl) > 0:
                            t = (llim[whl] - x[whl]) / wa1[whl]
                            alpha = numpy.min([alpha, numpy.min(t)])
                        whu = (
                            numpy.nonzero(((dwa1 != 0.0) & qulim) & ((x + wa1) > ulim))
                        )[0]
                        if len(whu) > 0:
